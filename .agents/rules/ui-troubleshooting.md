...
- **Symptom:** A tool title has a redundant icon, Color Picker shows a line
  under its titlebar, or a button gains an outline when the window opens.
- **Cause:** Decorative title content was added, Color Picker used the shared
  separator default, or a raw titlebar button retained the system focus effect.
- **Invariant:** Compact titlebars contain a text-only title. Color Picker uses
  shared custom actions with their focus effect disabled and disables the
  shared bottom separator without changing defaults for other tools.
- **Check:** Open every compact tool and inspect the initial focused state plus
  hover. Confirm there is no title icon, shared pill, or focus ring, and that
  Color Picker alone has no bottom separator.
