...
struct PortmanWindowView: View {
    private enum Page: String, CaseIterable { case local = "Local", forward = "SSH forwarding" }
    ...
    var body: some View {
        VStack(spacing: 0) {
            CompactTitlebar {
                CompactTitlebarTitle(title: "Portman")
            } actions: {
                if page == .local {
                    CompactTitlebarButton(title: "Refresh") {
                        Task { await service.refreshLocal() }
                    }
                }
            }

            Picker("Portman page", selection: $page) {
                ForEach(Page.allCases, id: \.self) { page in Text(page.rawValue).tag(page) }
            }
        ...
        .background(WindowAccessor(identifier: "portman"))
        .onAppear {
        ...
        .onChange(of: page) { if page == .local { selectedPortID = nil } }
        .onChange(of: host) {
            selectedRemotePorts = []
            localPortInputs = [:]
        ...
        VStack(alignment: .leading, spacing: 12) {
            HStack(alignment: .firstTextBaseline) {
                VStack(alignment: .leading, spacing: 2) {
                    ...
                    Text(cleanupMode
                         ? "\(selectedCleanupProcesses.count) processes selected"
                         : "\(service.localPorts.count) listening on ports 3000–9999")
                        .font(.system(size: 11)).foregroundStyle(.secondary)
                }
                Spacer()
                VStack(alignment: .trailing, spacing: 2) {
                    Text(ByteCountFormatter.string(fromByteCount: overviewMemory, countStyle: .memory))
                        .font(.system(size: 18, weight: .medium, design: .monospaced))
                        .monospacedDigit()
                    Text(String(format: "CPU %.1f%%", overviewCPU))
                        .font(.system(size: 11, design: .monospaced))
                        .foregroundStyle(.secondary)
                }
            }
            ProgressView(value: Double(overviewMemory), total: Double(ProcessInfo.processInfo.physicalMemory))
                .progressViewStyle(.linear)
                .accessibilityLabel("Server memory share of this Mac's RAM")

            if service.localPorts.isEmpty {
                ContentUnavailableView("No local servers", systemImage: "network",
                                       description: Text("Listening ports in the development range appear here."))
                ...
                HStack {
                    if cleanupMode {
                        ...
                        Spacer()
                        Button("Clean up…") { cleanupMode = true }
    ...
    private func localRow(_ port: PortmanLocalPort) -> some View {
        let attention = warning(for: port)
        return HStack(spacing: 8) {
            ...
            Button { selectedPortID = port.id } label: {
                HStack(spacing: 10) {
                    Text(":\(port.port)")
                    ...
                    VStack(alignment: .leading, spacing: 2) {
                        ...
                        Text(attention ?? "PID \(port.pid) · \(port.address)")
                            .font(.system(size: 11))
            ...
            .help("Show port \(port.port) details")
            ...
            if port.canStop && !cleanupMode {
                Menu {
                    Button("Stop process \(port.pid)…", role: .destructive) { pendingStop = port }
                } label: {
    ...
    private var overviewMemory: Int64 {
        ...
        return ports.reduce(0) { total, port in
            total + (seen.insert(port.pid).inserted ? port.memoryBytes : 0)
        }
    }
    ...
    private func warning(for port: PortmanLocalPort) -> String? {
        if port.memoryBytes >= 2 * 1024 * 1024 * 1024 { return "Memory above 2 GB" }
        guard let samples = service.history[port.id], let first = samples.first,
              let last = samples.last, last.memoryBytes - first.memoryBytes >= 500 * 1024 * 1024 else {
            return nil
        }
        let minutes = max(1, Int(last.date.timeIntervalSince(first.date) / 60))
        let growth = (last.memoryBytes - first.memoryBytes) / (1024 * 1024)
    ...
    private func localDetail(_ port: PortmanLocalPort) -> some View {
        ...
        return VStack(alignment: .leading, spacing: 16) {
            Button { selectedPortID = nil } label: {
            ...
            if let metadata = service.metadata[port.id] {
                ...
                if let branch = metadata.branch { detailRow("Branch", branch) }
            }
            ...
            HStack {
                ...
                Spacer()
                Text(ByteCountFormatter.string(fromByteCount: hovered?.memoryBytes ?? port.memoryBytes, countStyle: .memory))
            ...
            detailRow("CPU", String(format: "%.1f%%", hovered?.cpuPercent ?? port.cpuPercent))
            if let hovered {
            ...
            HStack {
                ...
                if port.canStop {
                    Button("Stop process…", role: .destructive) { pendingStop = port }
                }
            }
            .controlSize(.small)
            if let error = service.controlError { errorText(error) }
    ...
    private var forwardingPage: some View {
        VStack(alignment: .leading, spacing: 14) {
            ...
            if !service.tunnels.isEmpty {
                QuietDivider()
                Text("Tunnels").font(.system(size: 13, weight: .medium))
...
struct PortmanSettingsView: View {
    var body: some View {
        VStack(alignment: .leading, spacing: 12) {
            Text("PORTMAN").utilitySectionHeader()
            Text("Watch local development ports and open private server ports through SSH.")
                .font(.system(size: 13))
            Text("Local scanning runs while the Portman window or menu tab is open. SSH tunnels stay active until stopped or MacPowerToys quits.")
                .font(.system(size: 11)).foregroundStyle(.secondary)
