...
final class DevSyncPolicyTests: XCTestCase {
    ...
    func testEverythingModeSkipListIsTheOnlyFilter() {
        ...
        let visibleUntracked = engine(gitTracked: tracked, gitManifest: tracked.union(["vendor/untracked.o", "notes/todo.md"]))
        XCTAssertEqual(visibleUntracked.decide(DevPolicyInput(relativePath: "vendor/untracked.o", kind: .file, size: 1, isInsideGitDirectory: false)).reason, .commonExclusion)
        XCTAssertEqual(visibleUntracked.decide(DevPolicyInput(relativePath: "notes/todo.md", kind: .file, size: 1, isInsideGitDirectory: false)).reason, .gitTracked)
