...
|---|---|---|---|
| Open | Replace the current NetToys icon with a clearer product icon. | The first 50-candidate sheet contains 25 distinct Scan, SSH Anchor, History, Routing, and Network Hardware glyphs. Each glyph has a color study and a neutral production-family study. The production `NetToysLogo` asset stays unchanged during selection. | Select one `NT` candidate and install its geometry as Midnight and Porcelain assets. |
...
| Verify | Keep the System Monitor icon corners rounded at every size. | The OSM13 tile now clips every waveform band to the same 112pt rounded square as its ground. The same guard now covers the only other unguarded system tile, System Care. | Confirm the launcher, sidebar, and Dock icons in the latest installed build. |
