...
|---|---|---|---|
| Verify | Use three destinations: IP Scanner, SSH Anchor, and Network History. | `6b837a6` adds exactly these three sidebar destinations in a compact 220-point native workspace. A normal debug build showed all three pages and completed a loopback scan. | Confirm all three pages in the exact installed build. |
| Done | Reimplement the useful Angry IP Scanner workflow without copying GPL source, text, or art. | `13be002`, `6b837a6`, and `84a8533` add bounded native IPv4 scanning, single/range/CIDR/list/random/file targets, configurable ports and concurrency, live progress, sorting, filters, search, selection rescans, cancellation, details, comments, favorites, safe openers, history, and CSV, text, XML, IP-port, and SQL export. The Raycast build and 21 focused NetToys tests pass. | None for the requested native workflow. |
| Open | Complete the remaining clean-room Angry IP Scanner parity inventory. | The independent research inventory is in `docs/research/nettoys-angry-ip-ssh-anchor.md`. | Add hostname and per-target-port file input, IPv6 capability states, TTL and packet loss, HTTP and proxy detection, NetBIOS, editable protocol fetchers and openers, selectable columns, saved-result import, result deletion and navigation, CLI export, and native extension points. Never copy the GPL implementation or plugin ABI. |
...
| Verify | Track gateway and internet availability by active network. | `8e648a7` records only network, gateway, and internet transitions in a bounded local history. The transition and retention tests pass. | Confirm live disconnect and recovery events in the installed build. |
| Verify | Require a bundled login helper whenever NetToys is enabled. | `8e648a7` embeds `MacPowerToysNetHelper.app` under `Contents/Library/LoginItems`. `74ecb5f` makes enablement transactional through `SMAppService`, requires enabled status and a fresh helper heartbeat, unregisters on disable, and provides no main-app monitor fallback. Helper and embedded-app builds pass. | Verify registration, heartbeat, app-quit survival, and unregister behavior in the exact installed build. |
| Verify | Integrate NetToys with MacPowerToys. | `6b837a6` adds the launcher, scene, deep link, icon, stable window identifier, and Dock icon. `b61b0b3` adds a built Raycast launcher. Registry, routing, icon, and window tests pass. | Install the exact clean HEAD and verify the deep link, Raycast command, saved frame, and embedded source commit. |
...
| Open | Replace the rejected I21 icon with a simple OSS-derived mouse icon. | A 50-candidate review sheet uses pinned SVGs from Lucide, Tabler Icons, Phosphor Icons, Bootstrap Icons, and Google Material Icons. Every source has an ISC, MIT, or Apache-2.0 license. The production `InputDevicesLogoA` asset stays unchanged during selection. | Select one candidate, install it in the shared asset, and retain its upstream license notice. |
