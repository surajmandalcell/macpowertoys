...
| Verify | Make charts readable and responsive to interaction. | Treemap hover shows the name, size, and share even for tiny blocks. Rings highlight hovered segments and show names in the center. Chart navigation crossfades and scales, with Reduce Motion support. Run `36138597977` passed focused unit and focus tests; its dark and light renders show shallow rings using the chart height. Run `36140917852` captured active treemap hover and passed folder drill-down. It found that the ring chart identifier attached to the small center label, so pointer automation hit the hole; the chart now exposes one full-frame accessibility element. | Rerun hosted ring hover and inspect the highlighted segment and label. |
