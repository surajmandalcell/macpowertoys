...
| Verify | Make charts readable and responsive to interaction. | Treemap and ring hover highlight the target and show its name and size in a reserved footer below the plot, including tiny segments. The ring center retains the current folder. Chart navigation crossfades and scales, with Reduce Motion support. Run `36141822640` passed the earlier hover and drill checks; the revised footer UI check is pending. | Inspect motion and interaction in the final signed window. |
...
| Status | Request | Evidence | Remaining work |
|---|---|---|---|
| Verify | Stop live boxes snapping and keep the hover detail below the chart. | Treemap tile identity, its bounded item set, and split topology stay stable as weights cross; individual frames interpolate and zero-size skeleton entries appear in the first snapshot. Rings use stable bounded membership and animated arcs. Both charts reserve a 40-point detail row below the plot. The membership and topology regressions passed in hosted unit run `36162099059`; its light and dark renders show both footers. | Confirm motion in the final signed app. |
