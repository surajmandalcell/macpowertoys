...
| Status | Request | Evidence | Remaining work |
|---|---|---|---|
| Verify | Stop live boxes snapping and keep the hover detail below the chart. | Treemap tile identity, its bounded item set, and split topology stay stable as weights cross; individual frames interpolate and zero-size skeleton entries appear in the first snapshot. Rings use stable bounded membership and animated arcs. Both charts reserve a 40-point detail row below the plot. The membership and topology regressions, focused UI navigation, and refreshed light and dark renders passed in hosted run `36165658638`. | Confirm motion in the final signed app. |
