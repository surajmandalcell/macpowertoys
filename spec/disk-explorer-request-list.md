...
| Pending choice | Use a selected final tool icon. | Six light and dark SVG options are in `tmp/disk-explorer/index.index2.html`; the tool uses an SF Symbol and Raycast uses the app icon until one is chosen. | Owner selects an option; then promote it to the asset catalog and Raycast command. |
