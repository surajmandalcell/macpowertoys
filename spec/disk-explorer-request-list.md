...
|---|---|---|---|
| Verify | Make Modify a direct partition workspace and test native merge safety. | The page now shows a selectable textured disk map, volume rows, and grouped direct actions with visible reasons when an operation is unavailable. ExFAT resize is explicitly unavailable; the resize sheet reads macOS's current, minimum, and maximum limits before enabling review. Merge review names both partitions and whether the first survives; all destructive actions require the reviewed disk ID. On the authorized SD card (serial `0x19302912`, 15,634,268,160 bytes), HFS+ shrank to 3 GB and grew back while a 1 MB file retained SHA-256 `30e14955...`; an adjacent HFS+ merge preserved the same file and passed `verifyVolume`. Add and delete partition succeeded. ExFAT forced merge erased its marker and passed `verifyVolume`. A dry forced merge unexpectedly returned exit code 0 with “Merge canceled”; the command runner now sends the explicit confirmation after typed review and rejects that false-success result. The card is restored to one mounted `DISKMAN` ExFAT volume, and `fsck_exfat` reports it is OK. | Run hosted UI, render, and safety tests; inspect the signed installed build. |
