...
| Verify | Prevent transient or persistent titlebar focus outlines. | Shared titlebar controls suppress focus effects, and Color Picker now routes initial focus to its invisible window accessor like every compact applet. | Fresh-open the normal build, change window focus, and confirm no outline appears or remains. |
...
| Verify | Vertically align the complete titlebar row, lower traffic lights, remove zoom, and reclaim its title space. | Color Picker uses one 4pt row inset, a 22pt centerline, a 6pt traffic-light shift, a 60pt title start, hidden zoom, and a fixed root size. | Compare rendered midpoints within 1pt, confirm no empty third-control gap, and verify resizing fails. |
