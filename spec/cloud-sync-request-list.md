...
| Verify | Pause and resume a transfer directly from the menu-bar Cloud Sync tab. | Every active transfer row shows a state-aware Pause or Resume control wired to the existing safe job-manager actions. `3e36ad8` also gives the native menu-bar item an explicit accessibility name. The complete test suite and signed Release build pass. A controlled 19.5 MB transfer completed before interaction because rclone used its optimized local Copy path. | Use a disposable transfer that cannot use server-side Copy. In the physical menu bar, select Pause, confirm Paused and Resume, then select Resume and confirm Pause returns. The current desktop driver cannot inspect the SystemUIServer menu-extra tree. |
