...
| In progress | Preserve the reference's cleanup and process-tree actions, with protection and accurate reclaimed-memory preview. | Stop checks same-user identity and process start before SIGTERM, and checks child identity and parent before signaling. The cleanup estimate deduplicates process identities and leaves alerting servers unselected. Cleanup suggestions cover deleted folders, four observed idle hours, and three running days by default; stopping still requires confirmation. Detail offers Restart only when the same process still exposes a runnable executable, arguments, environment, and folder. Restart rechecks identity, waits for the listener to release, and writes output to a Portman log. Hosted run `36107019320` captured the rclone server's environment and successfully relaunched its listener on the same port. | Verify long-running selection and selected cleanup estimate in hosted tests, then confirmation in the final signed UI. The reference's Off, Ask, and Automatic cleanup policies remain to be adapted safely. |
...
passed at `cc3f465` with 861 passed, five skipped, and zero failures. It ran
an actual local SSH tunnel carrying HTTP traffic, listener-restart and idle-scan
tests, scanner and alert-policy tests, and a
...
`codesign --verify --deep --strict`. Its embedded source revision matched
committed `f2f29a8` at build time.
