...
|---|---|---|---|
| In progress | Match the server rows to the memory bar, sort by Port, Memory, Name, or CPU, and replace Alerts with searchable Settings. | Hosted run `36175966070` passed the Portman UI job and unit-test step. Its normal-mode four-listener capture shows the footer fully inside the panel, and the row and memory bar accessibility frames match. The same job exercised sorting, Forward navigation, and individual Settings filtering; the filtered capture shows the panel ending just below its controls. Alerts delivery and presentation are removed, while high-usage cleanup protection remains. | Install the exact signed commit without local UI tests. |
