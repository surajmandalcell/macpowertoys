...
|---|---|---|---|
| In progress | Refine the SSH scan loading state and related Forward layout. | Owner screenshot shows a detached spinner, duplicate status, and finished-result controls while a scan is still running. | Implement, inspect dark and light hosted captures, and exercise scan completion, empty, error, and cancellation without taking focus. |
