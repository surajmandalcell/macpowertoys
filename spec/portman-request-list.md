...
| In progress | Keep menu-bar server count and tunnel status current without unnecessary background work. | The Portman tab scans while visible and the window scans while open; tunnels persist while the app runs. A late manual scan now checks for a remaining surface owner before publishing results or errors after close. The app compiles without launch. | Add a count-bearing status item for full reference parity without scanning while unused. |
