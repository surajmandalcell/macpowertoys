...
|---|---|---|---|
| In progress | Refine the SSH scan loading state and related Forward layout. | The scan now has one inline status row and Cancel action; the completed result appears only after SSH succeeds. Hosted run `36240978463` passed the unit suite and its dark/light loading captures show aligned progress without a false result. Its 300pt minimum left blank space below loading, so the subsequent compact Forward pass lowers that minimum to 230pt, hides inactive actions, and collapses duplicate empty guidance. Run `36241819782` captured the correct invalid-host error, exposing an accessibility-query mismatch; the error now has a stable identifier. The revised source passed a local compile-only build. | Inspect final dark/light and idle captures, rerun the hosted invalid-host and navigation test, and verify scan success, empty results, and cancellation without taking focus. |
