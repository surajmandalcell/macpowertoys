...
|---|---|---|---|
| In progress | Fix the live-reviewed tab, remote identification, password, and Settings defects. | The source now uses an immediate tab line, removes the password button, probes non-interactive privileged `ss`, Docker, and systemd during a scan, and gives Settings full-width search plus compact trailing controls. Read-only `oci2` inspection confirmed that ordinary `ss` hid most process owners while the added sources exposed them. Hosted run `36185841165` passed Portman's tabs, Forward form, search, scan-interval and cleanup-mode menu interactions. Runs `36186473741` and `36187298927` reached the editor selector, but their cleanup input check prepended `6` to `4` because it did not select the existing value first. The field now commits a valid text draft on Return; the corrected selection test is pending. Compile-only Debug and signed Release builds passed. | Rerun the cleanup field UI check, sign and install a clean final build, and exercise the installed controls if native Computer access is approved. |
