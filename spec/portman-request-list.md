...
|---|---|---|---|
| In progress | Match active-forward Link and Stop control sizes, remember selected menu-bar tabs, and remove excess bottom space from short tab pages. | Owner screenshot shows the Link control taller than Stop on a running tunnel row; owner reports Portman returns to the wrong tab after reopening and blank space on other tab pages. | Implement and verify the affected panels in hosted UI checks and the signed installed app. |
