...
|---|---|---|---|
| In progress | Fix the live-reviewed tab, remote identification, password, and Settings defects. | The source now uses an immediate tab line, removes the password button, probes non-interactive privileged `ss`, Docker, and systemd during a scan, and gives Settings full-width search plus compact trailing controls. Read-only `oci2` inspection confirmed that ordinary `ss` hid most process owners while the added sources exposed them. Hosted run `36188072379` passed Portman's tabs, Forward form, search, editor, scan interval, cleanup mode, valid and invalid cleanup number edits, and selection reset. The Portman parser and SSH integration assertions passed in hosted runs; unrelated Switch and scroll-indicator assertions caused earlier full-suite failures. After confirming zero Cloud Sync jobs, the signed app and helper at clean commit `d30b7a1` were installed, passed strict signature verification, and the `/Applications` app launched in the background. A later read-only stamp check confirmed that revision remains installed; later commits did not change Portman source. | Native Computer inspection of the installed panel remains blocked: automatic approval review returned “Computer Use was not approved to use MacPowerToys.” |
