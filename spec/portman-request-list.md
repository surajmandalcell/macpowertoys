...
|---|---|---|---|
| In progress | Fix the live-reviewed tab, remote identification, password, and Settings defects. | The source now keeps each tab underline fixed and fades its opacity and label emphasis over 0.16 seconds, removes the password button, probes non-interactive privileged `ss`, Docker, and systemd during a scan, and gives Settings full-width search plus compact trailing controls. Read-only `oci2` inspection confirmed that ordinary `ss` hid most process owners while the added sources exposed them. Hosted run `36188072379` passed Portman's tabs, Forward form, search, editor, scan interval, cleanup mode, valid and invalid cleanup number edits, and selection reset before the fade correction. The new motion source passed a compile-only Release build. The installed signed app remains at `d30b7a1`. | Run hosted interaction checks for the new motion, install the final clean revision, and inspect the installed panel when native Computer access to MacPowerToys is approved. |
