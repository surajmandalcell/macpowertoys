...
|---|---|---|---|
| Verify | Remove launcher category labels and use the selected Minimal page header. | The sidebar is one flat tool list. Tool cards and detail pages no longer show category names. Every detail page now uses one compact identity and action row, left-aligned quiet tabs, and a body-level description. The layout detector reports no findings, and the debug test build succeeds. | Check every built-in and marketplace tool in the latest normal installed build. Confirm that long names and descriptions stay readable and that no category label remains. |
| Verify | Let background-capable tools show a separate menu bar icon. | Cloud Sync, Awake, Color Picker, Text Extractor, and Input Devices have a saved per-tool toggle. Native status items use stable autosave names for macOS position restoration. Disabling a tool removes its item. Power Stats keeps its existing grouped and individual metric controls. | In the latest normal installed build, enable each icon, move it, relaunch the app, and confirm its position and action. |
...
| Verify | Double-clicking the menu-bar icon opens the main window without opening the popover. | The status-item monitor now intercepts left mouse-down and mouse-up. It delays the single-click action by `NSEvent.doubleClickInterval` and consumes mouse-up so the system action cannot open early. A second click cancels the pending action and opens the main window. | Check one click and a slow and fast double-click in the latest normal installed build. |
...
| Verify | Use a darker menu-bar popover with a slight border around the complete tab group. | The popover applies an appearance-aware black shade. The tab group uses a 4pt inset, concentric 12pt outer and 8pt inner radii, and a neutral contrast-aware 1pt stroke. | Inspect light, dark, and Increased Contrast appearances in the latest normal installed build. |
