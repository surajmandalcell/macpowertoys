...
| Done | Keep workspace actions compact and aligned, and add restrained motion across apps. | `WorkspacePage` puts all actions in one centered 24pt row. Every window applies the Reduce Motion policy. The normal signed `1666d71` build confirmed the shared action row and the bottom-edge System Care status transition. The Reduce Motion regression test passes. | None. |
