...
| Open | Give the menu-bar icon a separate right-click menu containing only Open and Quit with icons. | The status item monitors left mouse-down only. Open and Quit exist only in the popover footer. | Add the compact native context menu if this remains required after double-click verification. |
| Partial | Restore every window to its last position, monitor, and Space. | `WindowStateManager` stores size, position, and display identity for every built-in window. No code records or restores a macOS Space. | Investigate native window restoration for Space placement, then verify across relaunch and multiple displays. |
| Verify | Keep padding, margins, titlebar alignment, body offsets, and card gutters consistent. | Shared layout tokens, compact titlebar components, aligned launcher gutters, and troubleshooting checks exist. Repeated corrections show source inspection alone is insufficient. | Run one final visual matrix on the latest normal build before release. |
...
| Done | Use compact custom titlebars without bottom borders, align traffic lights and actions, hide zoom, and float applet settings at bottom-right. | Shared `CompactTitlebar`, `WindowAccessor`, and `FloatingSettingsButton` enforce the current chrome rules. |
