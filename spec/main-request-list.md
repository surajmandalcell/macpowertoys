...
| Done | Keep branded sidebar icons in their original colors when selected. | `SidebarRow` keeps custom artwork in original rendering mode. The normal signed `d1cf9e2` build exercised all ten branded launcher selections. AI History, Ruler, Color Picker, Text Extractor, Cloud Sync, Logs, Awake, Input Devices, System Care, and Power Stats kept their icon colors. | None. |
| Done | Reduce structural divider strength everywhere without changing native menu separators. | All visual SwiftUI dividers route through the contrast-aware shared quiet divider. The post-change search leaves only the shared implementation and native command-menu or context-menu separators. The normal signed `d1cf9e2` build confirmed quiet separators across every app family. | None. |
| Done | Keep workspace actions compact and aligned, and add restrained motion across apps. | `WorkspacePage` gives actions one 24pt centerline. Every window applies the Reduce Motion policy. The normal signed `d1cf9e2` build confirmed aligned actions and page changes across every app family. The Reduce Motion regression test and 12 signed light and dark launch checks pass. | None. |
...
| Done | Always use the latest committed build across concurrent tasks. | The troubleshooting rule and `Makefile` reject dirty or stale installs. A normal signed Release build was installed from a clean `HEAD` after all 396 unit tests passed. The installed source matched `HEAD`, and strict code-sign verification passed. | Repeat this gate after a later source change. |
