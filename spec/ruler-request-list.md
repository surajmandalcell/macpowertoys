...
| Done | Match the FreeRuler color panel. | The upstream color-well behavior, zero-corner anchoring, and alpha-disabled color panel are preserved with focused tests. The app now positions the shared panel after AppKit restores its saved frame, which keeps the panel with Ruler Settings on the correct display. | Exercise it in the final installed build. |
