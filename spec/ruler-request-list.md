...
|---|---|---|---|
| Verify | Keep the native titlebar visible and aesthetically continuous in Ruler Settings and Defaults. | Both AppKit controllers install the same active HUD material behind their transparent native titlebars instead of a generic window-background slab. | Inspect both windows in the latest normal signed build. |
