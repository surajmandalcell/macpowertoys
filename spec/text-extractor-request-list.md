...
| Done | Remove compact-titlebar bottom borders and app icons throughout the applets. | The shared `CompactTitlebar` renders no separator or icon and is used by Text Extractor, Color Picker, Awake, and Ruler. The normal signed `98f35f6` build confirmed the borderless result in all four windows. | None. |
| Done | Vertically align the complete titlebar row with the traffic lights. | The 40pt titlebar applies one 4pt row inset; titles and 24pt actions use a 22pt centerline, and close/minimize move down 6pt. Unit coverage and the normal signed `98f35f6` build confirmed the alignment in all four windows. | None. |
